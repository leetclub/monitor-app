...
|---|------|----------|--------------------|------------|
| **1** | POST urgent operator task → Received | `POST /api/v1/tasks/urgent-operator` | **Wired** — GO CHECK → TM Received; Slack/mailto fallback | Confirm Live samples if errors appear |
| **2** | POST message to scheduled operator | `POST /api/v1/direct-messages` | **Wired** — Call OP modal sends TM DM | — |
...
| **4** | POST cleaning-overdue → inbox | `POST /api/v1/notifications/cleaning-overdue` | **Wired** — Cleaning alert modal Workflow channel | — |
