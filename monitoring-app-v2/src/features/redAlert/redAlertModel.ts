...
function machineIsExcludedFromRow(row: RedAlertRow): boolean {
  const mn = String(row.machineName ?? '');
  for (const m of EXCLUDED_NAME_MARKERS) {
    if (mn.includes(m)) return true;
  }
  return false;
}
...
export function freqColumnHeading(mode: RedAlertCompareMode): { title: string; sub: string } {
  switch (mode) {
    case 'week':
      return { title: 'WTD / trend', sub: 'Vs prorated last week' };
    case 'sameWeekdayLw':
      return { title: 'Today / trend', sub: 'Same weekday last week' };
    case 'yesterday':
      return { title: 'Today / trend', sub: 'Yesterday (same elapsed)' };
    default:
      return { title: 'WTD / trend', sub: 'Kuwait week' };
  }
}
