...
| `/red-flags` | Red Flags | **Red Alert** snapshot; columns match `alert.theleetclub.com.xlsx` **Red Flags** row 1 (see `docs/alert-workbook-red-flags-tab.md`); **Send credit / Test credits** from `GET /api/alert/remote-credits/today-totals` (tier colors); **Vends resolved** + machine **cleaning windows** merged in that response when `machines=` lists snapshot IDs; **Last cleaning** uses snapshot time + Admin windows; remaining trailing columns **—** / **?** until wired; five **timespan presets**; ~1 min refresh. |
