...
| Date (UTC) | Summary |
|------------|---------|
| 2026-05-08 | **Admin — Machines:** **Name of Tech Responsible** / **Name of QA Responsible** = one string; machine profile uses **`adminFieldCell`** + **CSS grid** (core row, time pairs, TZ/priority) so fields align; **`label.adminFormLabel`** uses `!important` column layout where `<label>` remains. |
