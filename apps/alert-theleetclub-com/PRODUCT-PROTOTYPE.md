...
| Date (UTC) | Summary |
|------------|---------|
| 2026-05-08 | **Admin — Machines layout:** Core uses **three equal columns** (`repeat(3, 1fr)` ≥840px). **`adminDetails`** padding increased; opened summary gets a divider. Operator cards (**`adminOperatorCard`**) cap name width; **Add shift segment** / **Remove operator** use **`adminButtonRow`**. Technician / QA use **`adminSubsectionTitle`** (`h3`) + **`adminStaffVariant`** (QA block separated by border). Time rows capped (**`max-width` ~26rem**). |
