...
| `/performance` | Performance | Overview **line trajectory** (All / Top 5 / Lowest 5) with period presets (last week default, WTD, MTD, …), KPI boxes (deficit, achievement rate, growth), then multi charts. Product cups optional (faster location-only by default). |
| `/admin` → Targets | Admin · Targets | Location target by **KD or cups**; promote **multiple Vendon products** each with cups/revenue target + period. |
