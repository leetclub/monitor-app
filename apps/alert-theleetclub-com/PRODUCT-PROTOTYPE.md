...
| `/red-flags` | Red Flags | Compare presets; table + cards; **Live operator** — Task Manager name in metric box + attendance badge (Late / Absent / Missing); column **not sortable**; **one tap** → schedule + **icon-only** contacts (no email/phone in table or modal body). **Last / tx** — weekday + date + time (3 lines; sortable via Vendon timestamp). **Downtime** — Today mins + same-elapsed period + **trend % vs period**; tap → events + **projected loss** (skips 0-sales days for baseline). Sales / Target / **SX (Loc only; products in popup)** / Trend / QA popups; fleet bar on Today vs Yesterday adds **Yest. full day** KD + **−2d full day** KD + **vs −2d** %. Last clean + tech visit use snapshot/SafetyCulture. |
| `/overall` | Overall | Workbook columns; **sortable headers** (⇅/▼/▲) on all columns with real data; **Operator Activity** column; **Downtime** (Today mins · period · trend %); **Attendance** from Task Manager (`GET /api/alert/workflow/machine-attendance-map` + tap → `operator-schedule` modal); **Sales** uses `GET /api/alert/overall/daily-sales-elapsed` — **Today vs Yesterday** or **Yesterday vs Day Before** per compare preset; … |
...
| `/admin` → Targets | Admin · Targets | Location target once (KD or cups); **bulk-apply location target** to many machines (products unchanged). Promote **1..N Vendon products** per location, each with its own target. Fleet table = **one row per location** (expand for products). Cached sales **insights** on hover-click. |
| `/admin` → Promo | Admin · Promo | Product assignment (owner or machine) from **Vendon product dropdown** (same catalog as Targets; loads on open, refreshes when machine/owner changes), calendar **day cup targets**, swipe instrument names. Separate from KD/SX Targets. |
...
*Figure 1 — **Classic:** coral Stitch sidebar. **Pro:** light iPad tab strip (unchanged). **v2 (`/v2`):** Manus Fleet Intelligence pages — separate from Pro (see figure 7).*
...
*Figure 4 — Settings header; **Sections** sidebar includes **Targets** (location KD + SX product) separate from Machines.*

![5 QA Visit](docs/product-prototype/figures/wire-05-qa-visit.svg)

*Figure 5 — QA Visit fleet / machine workspace.*
...
| 2026-08-03 | **Downtime revenue-loss calculator:** Projected Loss = baseline hourly KD × downtime hours × peak multiplier (+ spoilage). Baseline = yesterday same-elapsed ÷ hours; Kuwait peak bands (off-peak ×0.35 … peak 09–14 ×1.9). Modal shows Revenue baseline / Opportunity / Spoilage / Final impact / Volume. Same-clock observed sales kept as reference. |
...
| 2026-07-18 | **Performance growth labels:** KPI boxes show **% of prior period** / **% of last year** (index) plus signed **Growth** (e.g. 98.8% → −1.2%). Compare modal shows both columns. Classic/Pro/`/v2` share the same board. |
...
| 2026-07-15 | **Light / Dark for Classic + Pro:** Independent color mode toggle (Light · Dark) beside Classic · Pro. Tokens/fonts/surfaces follow `data-mode`. Defaults: Classic→dark, Pro→light until user picks. |
...
| 2026-07-15 | **QA Visit — reliability + co-located machines:** Location-only SC site visits now attribute to **all matching fleet machines** (siblings no longer starved by exclusive award). Title/header machine fields drive exclusive match. Default fleet range **90 days**; ingress **300s** timeout; partial SC scans surface **warning + Retry** (not silent empty). |
| 2026-07-15 | **QA Visit — SC pagination + errors:** people-api paginates SafetyCulture search at the real **100/page cap** (fixes missed recent visits in dense windows). Summary scan uses chunked windows. Fleet tab surfaces API errors instead of silent empty tables. |
