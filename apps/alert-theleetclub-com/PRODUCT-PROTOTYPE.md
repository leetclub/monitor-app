...
| `/v2` … `/v2/*` | **Alert v2** | Pure Manus **Fleet Intelligence** UI at `alert…/v2` (KPI cards, exception cards, fleet cards — **no Classic/Stitch dashboard UI**). Live Alert APIs. Classic + Pro stay at `/`. Login `/v2/login`. |
...
| Date (UTC) | Summary |
|------------|---------|
| 2026-07-15 | **Alert v2 pure Manus display:** `/v2` no longer mounts Classic pages. Exception cards, fleet cards, QA workspace, trajectory, Admin Manus tabs — Manus UI only + live APIs. Classic/Pro untouched at `/`. |
