...
| Date (UTC) | Summary |
|------------|---------|
| 2026-05-07 | **Overall:** Operating hours column **Admin · Location hours** (Alert machine profile). **Vendon-empty fallback** + snapshot columns wired where possible: **Last cleaned**, **Last vend failed** (counts), **Most issue** (reason), **Last transaction** (KWT). Banner when fleet rows exceed Red Flags snapshot count. |
