...
| Date (UTC) | Summary |
|------------|---------|
| 2026-05-08 | **Admin — Machines:** Technician / QA **Name of Tech Responsible** / **Name of QA Responsible** = **one text string** (not two inputs); visit days + hours; caption + `adminStaffInputRow` layout (Remove beside field) to fix alignment vs global `label` styles. Core/time rows use `align-items: flex-start`. |
