...
| `/red-flags` | Red Flags | Compare presets; table + cards; **Live operator** — Task Manager name in metric box + attendance badge (Late / Absent / Missing); column **not sortable**; **one tap** → schedule + **icon-only** contacts (no email/phone in table or modal body). **Last / tx** — weekday + date + time (3 lines; sortable via Vendon timestamp). **Downtime** — Today + selected period (Vendon OFF operational time); tap → events + **projected loss calculator** (baseline × hours × peak). Sales / Target / **SX (Loc only; products in popup)** / Trend / QA popups; fleet bar on Today vs Yesterday adds **Yest. full day** KD + **−2d full day** KD + **vs −2d** %. Last clean + tech visit use snapshot/SafetyCulture. |
| `/overall` | Overall | Workbook columns; **sortable headers** (⇅/▼/▲) on all columns with real data; **Operator Activity** column; **Downtime** (Today · period); **Attendance** from Task Manager (`GET /api/alert/workflow/machine-attendance-map` + tap → `operator-schedule` modal); **Sales** uses `GET /api/alert/overall/daily-sales-elapsed` — **Today vs Yesterday** or **Yesterday vs Day Before** per compare preset; … |
