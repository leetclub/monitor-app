...
| Date (UTC) | Summary |
|------------|---------|
| 2026-05-07 | **Red Flags — Today / Trend:** Column title **Today / Trend** with **three boxes**: **Score** (e.g. `2/d` or `N/WTD`), **Trend** (Δ% with ↑/↓/→, compact), **Gap** (distance to green: `0` good, `-N` bad, `—` unknown). **Green / red / neutral** boxes per Red Alert criteria (↑ worse, ↓ better; zero hits = green). Replaces the STALE/OFF/VEND mini-card column for this view. |
