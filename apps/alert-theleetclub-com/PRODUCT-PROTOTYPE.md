...
| Date (UTC) | Summary |
|------------|---------|
| 2026-05-07 | **Red Flags — Today / Trend:** **Three icon boxes** (Monitor-style): **Score** (`2/d` / `N/WTD`), **Trend** (Δ% with ↑/↓/→; **tiered red** on bad uptrends via `%` bands), **Variance** (distance to green: `0` / `-N` / `—`). Header subcopy **variance to green**. Whole-cell **tooltip** (baseline explanation + score / trend / variance lines). Green / red / neutral boxes per Red Alert rules. |
