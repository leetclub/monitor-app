...
| Date (UTC) | Summary |
|------------|---------|
| 2026-05-08 | **Admin — Machines:** Technician / QA use **one combined field** (“Name of Tech · Responsible” / “Name of QA · Responsible”), visit days, and hours; saved JSON `{ name, days[], windows[] }` with legacy `responsible`/`note` merged into `name` on load. **Machine profile form** uses stacked labels (`adminFormLabel`) so inputs align with cleaning/operator/time rows (fixes overlap from global inline labels). |
