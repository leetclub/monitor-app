...
function buildKeyFindings(opts: {
  ...
  if (rising[0]) {
    const p = rising[0];
    out.push(
      `Top riser: ${p.name} ${kd(p.prevRevenueKwd)} → ${kd(p.revenueKwd)} (${pctChange(Number(p.revenueKwd || 0), Number(p.prevRevenueKwd || 0))}; cups ${cups(p.prevCups)} → ${cups(p.cups)}).`,
    );
  }
  if (falling[0]) {
    const p = falling[0];
    out.push(
      `Top faller: ${p.name} ${kd(p.prevRevenueKwd)} → ${kd(p.revenueKwd)} (${pctChange(Number(p.revenueKwd || 0), Number(p.prevRevenueKwd || 0))}; cups ${cups(p.prevCups)} → ${cups(p.cups)}).`,
    );
  }
  if (lowMachines[0]) {
    const m = lowMachines[0];
    out.push(
      `Weakest vs machine target: ${m.machineName} at ${kd(m.periodKd)} of ${kd(m.locationTargetKd)} (${m.pctOfLocationTarget != null ? `${Math.round(Number(m.pctOfLocationTarget))}%` : '—'}).`,
    );
  }
  if (totalYoy > 0) {
    out.push(
      `YoY revenue ${pctChange(totalRev, totalYoy)} (${kd(totalRev)} vs LY ${kd(totalYoy)}); cups ${pctChange(totalCups, totalYoyCups)}.`,
    );
  }
  if (topProduct) {
    out.push(
...
export function downloadWeeklyProductReportPdf(opts: WeeklyPdfOpts) {
  ...
  const footerNote =
    'Revenue excludes WEB cashless / remote credit (customer sales only).';
  ...
  doc.save(`LEET-Weekly-Performance-Report-${stamp}.pdf`);
}
